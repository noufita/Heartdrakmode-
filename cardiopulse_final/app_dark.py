"""
CardioPulse — DARK mode + animated heart
...
st.set_page_config(page_title="CardioPulse · Dark", page_icon="🫀", layout="wide")
...
HEART_SVG = '<svg viewBox="0 0 24 24" width="22" height="22"><path d="M12 21s-1.45-1.32-3.6-3.3C5.4 15.36 2 12.28 2 8.5 2 6 4 4 6.5 4c1.74 0 3.04.81 3.91 2.09L12 7.6l1.59-1.51C14.46 4.81 15.76 4 17.5 4 20 4 22 6 22 8.5c0 3.78-3.4 6.86-6.4 9.2C13.45 19.68 12 21 12 21z" fill="#fff"/></svg>'
...
with st.sidebar:
    st.markdown(f'<div style="display:flex;align-items:center;gap:11px;">'
                f'<span style="width:42px;height:42px;border-radius:13px;background:{P};color:#fff;'
                f'display:flex;align-items:center;justify-content:center;box-shadow:0 0 22px -2px {P};" class="heart">{HEART_SVG}</span>'
    ...
    st.divider()
    st.caption("MODEL")
    ss.model = st.radio("Model", list(MODELS.keys()),
    ...
    st.markdown('<div class="card" style="margin-top:14px;font-size:11px;color:#B09C95;">Demo model · 70,000 records · educational use, not medical advice.</div>', unsafe_allow_html=True)
...
if ss.page == "Predictor":
    ...
    with c1:
        components.html(heart_html(prob, height=320), height=340, scrolling=False)
    with c2:
        st.markdown(f'<div style="font:700 12px Nunito;color:#B09C95;letter-spacing:.04em;">● LIVE · {MODELS[ss.model]}</div>'
...
# ---------------------------------------------------------------- DASHBOARD
    ...
    with g3:
        ...
        fig, ax = plt.subplots()
        ax.pie(gc, labels=gc.index, autopct="%1.1f%%", colors=[P,"#5a3a36"], textprops={"color":"#F2EDEA"})
    ...
    with h2:
        ...
        st.markdown(f'<div style="font-size:52px;font-weight:900;color:{P2};line-height:1;">'
                    ...
                    f'<span style="color:#B09C95;">accuracy · {len(BUNDLE["y_test"]):,} test</span>', unsafe_allow_html=True)
        p = st.columns(3)
